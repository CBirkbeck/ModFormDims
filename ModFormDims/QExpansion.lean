...
theorem z_tendsto (hh : 0 < h) :
  ...
  rintro ⟨u1, u2⟩
  have : NeZero h := ⟨hh.ne'⟩
  rw [← QZ_eq_id h hh.ne' _ u2] at u1
  have := abs_q_lt_iff h hh A (Z h q)
  rw [this] at u1
...
theorem eq_cuspFcn (hh : h ≠ 0) (z : ℂ) (hf : ∀ w : ℂ, f (w + h) = f w) :
  ...
  rw [this]
  dsimp only [cuspFcn0]
  obtain ⟨m, hm⟩ := ZQ_eq_mod_period h hh z
...
theorem cuspFcn_diff_at (hh : h ≠ 0)
  ...
  dsimp at hF
  have : cuspFcn h f ∘ Q h ∘ L = f ∘ L := by ext1 z; exact (eq_cuspFcn h f hh (L z) hf).symm
  rw [this] at hF
  have : z = L q := by
...
theorem F_diff_near_zero (hh : 0 < h) (h_hol : ∀ᶠ z : ℂ in atIInf', DifferentiableAt ℂ f z)
  ...
  refine' ((z_tendsto h hh).eventually h_hol).mp _
  apply eventually_nhdsWithin_of_forall; intro q hq h_diff
  rw [← QZ_eq_id _ hh.ne' _ hq]
  apply cuspFcn_diff_at _ _ hh.ne'
  exact h_diff
  exact hf
...
theorem F_bound (hh : 0 < h) (h_bd : IsBigO atIInf' f (1 : ℂ → ℂ)) :
  ...
  refine' IsBigO.congr' (h_bd.comp_tendsto <| z_tendsto h hh) _ (by rfl)
...
theorem F_diff_at_zero (hh : 0 < h) (h_bd : IsBigO atIInf' f (1 : ℂ → ℂ))
  ...
  have T:= (F_diff_near_zero h f hh h_hol hf)
  replace t := T.and t
...
theorem tendsto_at_I_inf (hh : 0 < h) (h_bd : IsBigO atIInf' f (1 : ℂ → ℂ))
  ...
  suffices Tendsto (cuspFcn h f) (𝓝[≠] 0) (𝓝 <| cuspFcn h f 0) by
    have t2 : f = cuspFcn h f ∘ Q h := by ext1; apply eq_cuspFcn h f hh.ne' _ hf
    conv =>
      congr
      rw [t2]
    apply Tendsto.comp; exact this
    apply tendsto_nhdsWithin_of_tendsto_nhds_of_eventually_within
    exact q_tendsto h hh;
...
theorem cuspFcn_zero_of_zero_at_inf (hh : 0 < h) (h_bd : IsLittleO atIInf' f (1 : ℂ → ℂ)) :
  ...
  have :IsLittleO (𝓝[≠] (0 : ℂ)) (cuspFcn h f) 1  (F := ℂ) := by
    refine' IsLittleO.congr' (h_bd.comp_tendsto <| z_tendsto h hh) _ (by rfl)
...
theorem exp_decay_of_zero_at_inf (hh : 0 < h) (h_bd : IsLittleO atIInf' f (1 : ℂ → ℂ))
  ...
  have : f = fun z : ℂ ↦ (cuspFcn h f) (Q h z) := by
    ext1 z
    exact eq_cuspFcn h _ hh.ne' _ hf
  rw [this]
  simp_rw [← abs_q_eq h, ← norm_eq_abs]
  apply IsBigO.norm_right
  apply (bound_holo_fcn (cuspFcn h f) ?_ ?_).comp_tendsto (q_tendsto h hh)
